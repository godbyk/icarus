...
CBlob::CBlob( const CBlob &src )
{
	...
	mean = src.mean;
	stddev = src.stddev;
	nombrePixelsExterns = src.nombrePixelsExterns;
...
- MODIFICATION: Date. Author. Description.
*/
void CBlob::FillBlob( IplImage *imatge, CvScalar color ) const					  
{
	...
	while( itEdges != vectorEdges.end() )
	{
		  ...
		  )
		{
			if( dinsBlob )
			{
				cvLine( imatge, *itEdges, *itEdgesSeguent, color );
			}
			dinsBlob =! dinsBlob;
		}
		itEdges++;
