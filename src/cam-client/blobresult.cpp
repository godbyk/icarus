	...
	switch(condition)
	{
		case B_EQUAL:
			for(i=0;i<numBlobs;i++, itavaluacioBlobs++)
			{
				...
				//Actuacio(filterAction,i,resultavaluacio, dst );
...
/**
- FUNCIÓ: Actuacio
- FUNCIONALITAT: Funció auxiliar de Filter. Segons l'acció de FilterResult, 
			   i depenent del resultat obtingut d'avaluar l'operació 
			   sobre l'element (obtingut a la funció Filter) afegeix o elimina  
			   el blob al resultat
- PARÀMETRES:
	- FilterResult:	B_INCLUDE,B_EXCLUDE
	- index: número de blob del que volem calcular el resultat.
	- ResultAvaluacio: si el blob ha superat la condició
	- dst: seqüència de blobs on estem deixant el resultat
- RESULTAT:
	- Afegeix el blob m_blobs[index] blob a dst si s'escau
- RESTRICCIONS:
- AUTOR: Ricard Borràs
- DATA DE CREACIÓ: 25-05-2005.
- MODIFICACIÓ: Data. Autor. Descripció.
*/
void CBlobResult::Actuacio(int FilterResult, int index, bool ResultAvaluacio, 
						   CBlobResult &dst ) const
{
	switch(FilterResult)
	{
		case B_INCLUDE:
			//si el resultat es TRUE, afegim el blob al resultat
			if(ResultAvaluacio)
			{
				dst.m_blobs.push_back( new CBlob( GetBlob( index ) ));
			}
			break;
		case B_EXCLUDE:
			//si el resultat es FALSE, afegim el blob al resultat
			if(!ResultAvaluacio)
			{
				dst.m_blobs.push_back( new CBlob( GetBlob( index ) ));
			}
...
void CBlobResult::PrintBlobs( char *nom_fitxer ) const
{
	double_stl_vector area, perimetre, exterior, mitjana, compacitat, longitud, 
					  numPixelsExterns, perimetreConvex;
	int i;
	FILE *fitxer_sortida;
	...
	numPixelsExterns = GetSTLResult( CBlobGetExternPixels());
	...
	{
		fprintf( fitxer_sortida, "blob %d ->\t a=%7.0f\t p=%8.2f\t pconvex=%8.2f\t ext=%.0f(%.0f)\t m=%7.2f\t c=%3.2f\t l=%8.2f\n",
				 i, area[i], perimetre[i], perimetreConvex[i], exterior[i],numPixelsExterns[i], mitjana[i], compacitat[i], longitud[i] );
	}
